...
function createIngredientsMarkup(ingredients = []) {
  ...
  return `
    <p class="menu-dish-ingredients">
      ...
      ${ingredients.map((item) => escapeHtml(item)).join(" • ")}
    </p>
  `;
}

function renderMenuDishCard(item, options = {}) {
  const { badge = "", mediaLabel = "VANATUR", compact = false } = options;
  const cardClass = compact ? "menu-dish-card menu-dish-card--compact" : "menu-dish-card";
          ...
          item.portion
            ? `
              <div class="menu-dish-meta">
                <span>${escapeHtml(item.portion)}</span>
              </div>
            `
            : ""
        }
  ...
  const renderPanel = () => {
    ...
    panel.innerHTML = `
      ...
      <div class="menu-dish-grid menu-dish-grid--compact">
        ${category.items
          .map((item) => renderMenuDishCard(item, { mediaLabel: category.name, compact: true }))
          .join("")}
      </div>
    `;
  };
          ...
          (category) => `
            <section class="menu-category-card reveal">
              ...
              <div class="menu-dish-grid">
                ${category.items
                  .map((item) => renderMenuDishCard(item, { mediaLabel: category.name }))
                  .join("")}
              </div>
            </section>
          `
